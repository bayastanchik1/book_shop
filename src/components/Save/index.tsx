...
function Save() {
  const { basketDate } = useBasketContext();
  ...
  const { delBasket,addBasket, readBasket, onBasket, getBaskets } = useBasketContext();
  const {getProducts , onProduct } = useProductContext()

  useEffect(() => {
    readBasket();
  }, []);

  console.log('product',onBasket);
